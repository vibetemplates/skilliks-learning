-- Migration: Create project_prompts table
...
CREATE TABLE project_prompts (
    ...
    CONSTRAINT fk_project_prompts_project FOREIGN KEY (project_id) 
        REFERENCES projects(id) ON DELETE CASCADE,
    CONSTRAINT fk_project_prompts_parent FOREIGN KEY (parent_prompt_id) 
        REFERENCES project_prompts(id) ON DELETE SET NULL,
    
    -- Indexes for better performance
    INDEX idx_project_prompts_project_id (project_id),
    INDEX idx_project_prompts_parent_id (parent_prompt_id),
    INDEX idx_project_prompts_status (status),
    INDEX idx_project_prompts_executed_at (executed_at),
    INDEX idx_project_prompts_created_at (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Add comment to the table
ALTER TABLE project_prompts COMMENT 'Stores prompts sent to development tools for implementing project features';