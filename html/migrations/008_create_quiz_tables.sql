...
CREATE TABLE IF NOT EXISTS `quiz_responses` (
  ...
  `question_id` int(11) NOT NULL,
  `answer_option_id` int(11) DEFAULT NULL, -- For multiple choice/true-false
  `answer_text` text, -- For short answer/essay/fill-blank
...
-- Update quiz total points when questions are added/updated/deleted
CREATE TRIGGER update_quiz_total_points_insert
...
CREATE TRIGGER update_quiz_total_points_update
...
CREATE TRIGGER update_quiz_total_points_delete
...
-- Update question bank usage count
CREATE TRIGGER update_question_bank_usage_insert
AFTER INSERT ON quiz_question_links
FOR EACH ROW
BEGIN
...
CREATE TRIGGER update_question_bank_usage_delete
AFTER DELETE ON quiz_question_links
FOR EACH ROW
BEGIN
    UPDATE question_bank 
    SET usage_count = usage_count - 1 
...
-- =============================================
-- This procedure will be used to migrate existing JSON quiz data to the new tables
DELIMITER //

CREATE PROCEDURE MigrateQuizData()
BEGIN
    DECLARE done INT DEFAULT FALSE;
    DECLARE lesson_id_var INT;
    DECLARE quiz_data_var JSON;
    DECLARE cur CURSOR FOR 
        SELECT id, quiz_data 
        FROM lessons 
        WHERE lesson_type = 'quiz' 
        AND quiz_data IS NOT NULL 
        AND JSON_LENGTH(quiz_data) > 0;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;

    -- This is a placeholder for the migration logic
    -- The actual migration will be done in PHP to handle complex JSON parsing
    SELECT 'Quiz data migration should be run from PHP migration script' AS message;
END//
...
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.quizzes TO 'your_app_user'@'localhost';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.quiz_questions TO 'your_app_user'@'localhost';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.quiz_answer_options TO 'your_app_user'@'localhost';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.quiz_attempts TO 'your_app_user'@'localhost';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.quiz_responses TO 'your_app_user'@'localhost';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.question_bank TO 'your_app_user'@'localhost';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.question_bank_answers TO 'your_app_user'@'localhost';
-- GRANT SELECT, INSERT, UPDATE, DELETE ON project_tracker.quiz_question_links TO 'your_app_user'@'localhost';